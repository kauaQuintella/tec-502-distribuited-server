...
import com.cardgame_distribuited_servers.tec502.server.game.inventory.TradeService;
...
public class ServerController {
    ...
    @PostMapping("/servers/test-open-pack")
    public ResponseEntity<String> testOpenPack(@RequestBody String idUser) {
        System.out.println("Recebido pedido de teste para abrir pacote...");
        Optional<Skin> acquiredSkin = skinsManager.abrirPacote(idUser);

        return acquiredSkin
                .map(skin -> ResponseEntity.ok(gson.toJson(skin)))
                .orElse(ResponseEntity.status(HttpStatus.NOT_FOUND)
                        .body("{\"error\":\"Erro ao abrir pacote. Utilizador não encontrado ou skins esgotadas.\"}"));
    }
    ...
    @PostMapping("/trade/propose")
    public ResponseEntity<String> proposeTrade(
            @RequestHeader("X-Player-ID") String fromPlayerId, 
            @RequestBody ProposeTradeRequest request) {
        return tradeService.proposeTrade(fromPlayerId, request);
    }

    @PostMapping("/trade/accept")
    public ResponseEntity<String> acceptTrade(
            @RequestHeader("X-Player-ID") String acceptingPlayerId, 
            @RequestBody AcceptTradeRequest request) {
        return tradeService.acceptTrade(acceptingPlayerId, request);
    }
}
